...
def getModelsNames():
    ...
    jsonModelNames = [{'label': key, 'value':item.id} for key, item in hModels.items()]
...
def getModelsInfo():
    ...
    for mName, mObject in hModels.items():
        jsonModelsInfo[mObject.id] = {
            'name': mName,
            'properties': {pObject.id: {'name': pObject.name, 'type': pObject.type} for pName, pObject in
                           mObject.schema.items()}
        }
        jsonModelsInfoList = [{"label": jsonModelsInfo['name'], 'value': item.keys} for item in jsonModelsInfo]
    # return jsonModelsInfo
    return jsonModelsInfo


# def getModelsWithFields(modelList, fieldList):
#
#     # output should be a list of dictionaries with [label: fieldname, value: fieldname]
#     return []
